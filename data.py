    ...
    if selected == "Explore Data":
        ...
        with tabm:
            ...
            with tab1:
                ...
                mycursor.execute(
                    f"SELECT State, Transaction_Amount FROM aggregate_transaction WHERE Year = '{year}' AND Quarter = '{quarter}' AND Transaction_Type = '{type}';")
                df = pd.DataFrame(mycursor.fetchall(), columns=['State', 'Transaction_Amount'])
                df_bar = df.set_index(pd.Index(range(1, len(df) + 1)))

                mycursor.execute(
                    f"SELECT State, Transaction_Count, Transaction_Amount FROM aggregate_transaction WHERE Year = '{year}' AND Quarter = '{quarter}' AND Transaction_Type = '{type}';")
            ...
            with tab2:
                ...
                mycursor.execute(
                    f"SELECT State, SUM(Count), Brand_Name FROM aggregate_user WHERE Year = '{yr}' AND Quarter = '{qtr}' AND Brand_Name= '{brand}' GROUP BY State , Brand_Name;")
        ...
        with tabc:
            ...
            if types == 'Transactions':
                ...
                with tab1:
                    ...
                    mycursor.execute(f"select State, sum(Transaction_Count) as Total_Transactions_Count, sum(Transaction_Amount) as Total_Transaction_Amount from aggregate_transaction where Year = {Years} and Quarter = {Quarter} group by State order by Total_Transaction_Amount desc limit 10")
                ...
                with tab2:
                    ...
                    mycursor.execute(f"select District, sum(Count) as Total_Count, sum(Amount) as Total_Amount from map_transaction where Year = {Years} and Quarter = {Quarter} group by District order by Total_Amount desc limit 10")
                ...
                with tab3:
                    ...
                    mycursor.execute(f"select Pincode, sum(Transaction_Count) as Total_Transactions_Count, sum(Transaction_Amount) as Total_Transaction_Amount from top_transaction where Year = {Years} and Quarter = {Quarter} group by Pincode order by Total_Transaction_Amount desc limit 10")
                ...
                with tab1:
                    ...
                    mycursor.execute(f"select Brand_Name, sum(Count) as Total_Users, avg(Percentage)*100 as Avg_Percentage from aggregate_user where Year = {Years} and Quarter = {Quarter} group by Brand_Name order by Total_Users desc limit 10")
                ...
                with tab2:
                    st.markdown("### :violet[District]")
                    mycursor.execute(f"select District, sum(Registered_Users) as Total_Users, sum(App_Opens) as Total_AppOpens from map_users where Year = {Years} and Quarter = {Quarter} group by District order by Total_Users desc limit 10")
                ...
                with tab3:
                    st.markdown("### :violet[Pincode]")
                    mycursor.execute(f"select Pincode, sum(Registered_Users) as Total_Users from top_users where Year = {Years} and Quarter = {Quarter} group by Pincode order by Total_Users desc limit 10")
                ...
                with tab4:
                    st.markdown("### :violet[State]")
                    mycursor.execute(f"select State, sum(Registered_Users) as Total_Users, sum(App_Opens) as Total_AppOpens from map_users where Year = {Years} and Quarter = {Quarter} group by State order by Total_Users desc limit 10")
...
def insights():
  if selected == "Insights":
    ...
    if questions == '1.Which district within a state has the lowest number of users over years and quartes?':
                    ...
                    Quarter = st.slider("Quarter", min_value=1, max_value=4)
                    mycursor.execute(f"""SELECT u.State, u.Year, u.Quarter, u.District, u.Registered_Users  FROM map_users u JOIN (
                                         SELECT State, Year, Quarter, MIN(Registered_Users) AS min_Users  FROM map_users  WHERE Year = {Year} AND Quarter = {Quarter}
                                         GROUP BY State, Year, Quarter ) min_users  ON u.State = min_users.State  AND u.Year = min_users.Year  AND u.Quarter = min_users.Quarter 
                    ...
                    st.plotly_chart(bar_chart) 
                      ...
                      mycursor.execute("""SELECT State,Pincode, MAX(Max_Registered_Users) AS Max_Registered_Users, MAX(Max_Transaction_Count) AS Max_Transaction_Count
                                            FROM ( SELECT State,Pincode, MAX(Registered_Users) AS Max_Registered_Users, NULL AS Max_Transaction_Count
                                            FROM phonepe.top_users GROUP BY State,Pincode UNION ALL
                                            SELECT State,Pincode, NULL AS Max_Registered_Users, MAX(Transaction_Count) AS Max_Transaction_Count
                                            FROM phonepe.top_transaction GROUP BY State,Pincode) AS combined
                                            GROUP BY State,Pincode ORDER BY MAX(Max_Registered_Users) DESC, MAX(Max_Transaction_Count) DESC
                                            LIMIT 10;""")
                      ...
                      st.plotly_chart(fig)
                        ...
                        year = st.slider("**Year**", min_value=2018, max_value=2023)
                        query = """SELECT State, SUM(Transaction_Count) AS Total_Transaction_Count,  SUM(Transaction_Amount) AS Total_Transaction_Amount
                                  FROM aggregate_transaction  WHERE Year = %s GROUP BY State ORDER BY State ASC;"""
                ...
                mycursor.execute("""SELECT State,Brand_Name, MAX(Percentage) AS High_Percentage FROM aggregate_user GROUP BY State,Brand_Name
                                    ORDER BY  High_Percentage DESC LIMIT 10;""" )
              ...
              mycursor.execute("""SELECT State, District, Registered_Users FROM ( SELECT  State, District, Registered_Users, ROW_NUMBER() OVER (PARTITION BY State ORDER BY Registered_Users DESC) AS row_num
                                        FROM phonepe.map_users) AS ranked_users  WHERE  row_num = 1;""")                              
             ...
             mycursor.execute("""SELECT Brand_Name,SUM(Count) AS Total_Users FROM aggregate_user GROUP BY Brand_Name ORDER BY Total_Users DESC LIMIT 10;""")
             ...
             st.plotly_chart(fig)
                           ...
                           mycursor.execute("""SELECT State, SUM(App_Opens) AS Total_App_Opens FROM map_users GROUP BY State ORDER BY Total_App_Opens DESC
                                                LIMIT 15;""")
                           ...
                           st.plotly_chart(fig)
                            ...
                            mycursor.execute("""SELECT au.State, au.Brand_Name, SUM(au.Count) AS Total_Transactions FROM aggregate_user au JOIN
                                                    aggregate_transaction at ON au.State = at.State  GROUP BY au.State, au.Brand_Name ORDER BY
                                                        Total_Transactions DESC limit 10;""")
            ...
            mycursor.execute("""SELECT  Year, SUM(Count) AS Total_Registered_Users FROM aggregate_user GROUP BY Year ORDER BY Year;""")
            ...
            st.plotly_chart(fig)
             ...
             mycursor.execute("""SELECT State, SUM(Transaction_Amount) AS Total_Transaction_Amount FROM aggregate_transaction  WHERE Year = 2023 GROUP BY State
                                        ORDER BY Total_Transaction_Amount ASC  LIMIT 15;""")
